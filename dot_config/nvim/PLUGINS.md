...
### mason.nvim + mason-lspconfig.nvim + nvim-lspconfig（`plugins/lsp.lua` + `lsp/`）
...
| ファイル | 役割 |
|---------|------|
| `lua/plugins/lsp.lua` | Mason / mason-lspconfig / mason-tool-installer プラグイン定義 |
| `lua/lsp/init.lua` | 全サーバー共通設定（`vim.lsp.config("*", ...)` / 診断）|
| `lua/lsp/lua_ls.lua` | Lua サーバー固有設定 |
| `lua/lsp/ruby.lua` | Ruby サーバー（追加設定なし・共通設定のみ）|
| `lua/lsp/typescript.lua` | TypeScript / JavaScript サーバー固有設定 |

#### 使用している LSP API（nvim-lspconfig v2 / Neovim 0.11+）

nvim-lspconfig v2 以降は以下の API を使う（旧 API は非推奨で起動時に警告が出る）:

```lua
-- 全サーバー共通設定（on_attach・capabilities）
vim.lsp.config("*", {
  capabilities = require("cmp_nvim_lsp").default_capabilities(),
  on_attach    = function(_, bufnr) ... end,
})

-- サーバー固有設定（省略可。追加設定がある場合のみ記述）
vim.lsp.config("lua_ls", {
  settings = { Lua = { diagnostics = { globals = { "vim" } } } },
})

-- サーバーを有効化（mason-lspconfig の handlers から呼ばれる）
vim.lsp.enable("lua_ls")
```

mason-lspconfig の `handlers` パターンでインストール済みサーバーを自動有効化している:

```lua
require("mason-lspconfig").setup({
  ensure_installed = { "lua_ls", "ruby_lsp", "ts_ls" },
  handlers = {
    function(server_name)     -- デフォルト: 全サーバーを有効化
...
nvim-lspconfig は「サーバーのコマンド・デフォルト設定・filetype マッピング」の定義を提供するために依存に含めているが、`require("lspconfig").server.setup()` は呼ばない。
...
| 操作 | キー | 動作 |
|------|------|------|
| トグル | `<Leader>e` | エディタ → neo-tree にフォーカス / neo-tree → エディタに戻る（トグル） |
| 現在ファイルを表示 | `<Leader>E` | 編集中ファイルをツリー内でハイライト（VS Code の「エクスプローラーで表示」相当） |
| 閉じる | `q`（neo-tree 内） | エクスプローラーを閉じる |
| フォーカスをエディタへ | `Ctrl+l`（neo-tree 内） | エクスプローラーを閉じずにエディタ側へ移動 |

> `<Leader>e` はトグル動作: エディタから押すと neo-tree へ、neo-tree から押すとエディタへ戻る。
> `<Leader>E` はエディタにフォーカスがある時に使う。neo-tree から押すと「Order by」メニューが開いてしまう。
...
neo-tree 内で `/` を押すと fuzzy_finder モードでフィルタリングできる。

| キー | 動作 |
|------|------|
| `/` | フィルター入力開始 |
| `Esc`（fuzzy_finder 内） | fuzzy_finder を閉じる |
| `q`（fuzzy_finder 内） | fuzzy_finder をキャンセル |
| `Esc`（ノーマル状態） | フィルターをクリアして全ファイル表示に戻る |
| `<` | フィルタークリア（代替手段） |
...
### pwntester/octo.nvim（`plugins/git.lua`）

GitHub の PR・Issue を Neovim 内で閲覧・操作・レビューできる。
事前に `gh auth login` で GitHub CLI のログインが必要。

#### グローバルキー（どのバッファからでも使える）

| キー | 動作 |
|------|------|
| `<Leader>po` | 現在ブランチの PR を開く |
| `<Leader>pp` | PR 一覧（Telescope） |
| `<Leader>pf` | PR 変更ファイル一覧 |
| `<Leader>pc` | CI チェック状態 |
| `<Leader>pr` | レビューを開始 |
| `<Leader>il` | Issue 一覧 |
| `<Leader>ic` | Issue 作成 |

#### PR バッファ内のキー（`\` = LocalLeader プレフィックス）
...
詳細は `<Leader>?5` のチュートリアル 5 を参照（`:Octo review submit` でレビュー提出）。
...
Vault 内の Markdown ファイルを開いた時のみ自動ロードされるが、
別リポジトリ（CFO-Alpha 等）でも `<Leader>o*` キーや `:Obsidian*` コマンドを実行すると
その場でプラグインがロードされ、Vault にアクセスできる。

| キー | 動作 |
|------|------|
| `<Leader>of` | Vault をディレクトリツリーで開く（snacks explorer）。explorer 内で押すと閉じて neo-tree を復元 |
...
### xvzc/chezmoi.nvim（`plugins/chezmoi.lua`）

chezmoi で管理している dotfiles の差分確認・追加・適用を Neovim 内で行う。
ソースディレクトリ（`~/.local/share/chezmoi/`）のファイルを開いた時と `<Leader>cz*` キーで自動ロード。
...
| `<Leader>cza` | 現在ファイルを add（目的ファイル → ソース） |
| `<Leader>czp` | apply（ソース → 目的ファイル） |
| `<Leader>czs` | status（変更サマリー） |
| `<Leader>czo` | ソースディレクトリをトグル。explorer 内で押すと閉じて neo-tree を復元 |

詳細な操作ガイドは `CHEZMOI.md` を参照（`<Leader>?c` で開く）。
...
| `<Leader>?c` | chezmoi 操作ガイドをフロートウィンドウで表示 |
